...
```{r 02-Data-Wrangling-15, warning=F}
library(dplyr)
library(tidyr)
library(stringr)
...
```{r 02-Data-Wrangling-21, warning=F}
...
```{r 02-Data-Wrangling-26-3-1, warning=F}
#view each image in the list
neurobase::ortho2(mca.list.nii[[1]])
```

In this example, the first 3 segmented images are averaged and viewed.

```{r 02-Data-Wrangling-26-3-2, warning=F}
#view average image
mca_ave3<-(mca.list.nii[[1]]+mca.list.nii[[2]]+mca.list.nii[[3]])/3
neurobase::ortho2(mca_ave3)
```

The output below is the same as above but is perforemd on arrays.

```{r 02-Data-Wrangling-26-3-3, warning=F}
...
```{r 02-Data-Wrangling-26-3-4, warning=F}
...
```{r 02-Data-Wrangling-26-5, warning=F}
...
```{r 02-Data-Wrangling-26-6, warning=F}
library(RNiftyReg)
...
```{r 02-Data-Wrangling-26-7, warning=F}
library(rgl)
library(misc3d)
library(MNITemplate)
library(aal)
