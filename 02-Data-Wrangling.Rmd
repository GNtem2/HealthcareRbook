...
```{r list files}
#list all files in a directory then
#list.files()
...
pca_mca<-prcomp(scale(m.listarray))


```

One way of handling imaging data for multivariate analysis is to flatten the image. This can be done by using _as.vector_ function.

```{r flatten 3D}
#flatten 3D
niivector = as.vector(img[,,])
...
#convert the list above to array
newmcaarray<-list2array(mca.list.nii)

dim(newmcaarray)
#[1]  91 109  91  29

mcavector = as.vector(newmcaarray[,,,])

mca.pca<-prcomp(newmcaarray, center = T, scale. = T)
...
Label a parcellated region in AAL atlas
