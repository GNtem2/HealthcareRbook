...
Arrays are arranged in matrix of rows and columns of a single data type. Later 
in this chapter, we will illustrate the importance of arrays for manipulating 
...
The _rapply_ function is applied to a list, contingent on the second argument. 
In this example, only numeric values are evaluated.
...
Measurement of volume requires knowing dimensions of voxel.

```{r 2-Data-Wrangling-26-1-1}
#measure volume
...
```{r 02-Data-Wrangling-26-3, warning=F}
library(oro.nifti)
library(abind)
library(CHNOSZ)
library(RNiftyReg)
...
One way of handling imaging data for multivariate analysis is to flatten the image. 

```{r 02-Data-Wrangling-26-4, warning=F}
#flatten 3D
...
(tgz file), _untar_, _zip_ (gz file) and _unzip_ function are from the _utils_ library. The _readNIfTI_ call can open gz file without the need to call _unzip_ function.

```{r 02-Data-Wrangling-26-5}
