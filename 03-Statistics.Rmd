...
```
We provided an example below for generating power of clinical trial. Examples are taken from a paper  sample size estimation for trials [@pmid16931782].
```{r proportion}
library(pwr)
#h is effect size. effect size of 0.5 is very large 
#sample size
pwr.2p.test(h=0.5,n=50,sig.level=0.05,alternative="greater")

#medium effect size
pwr.2p.test(h=0.1,n=50,sig.level=0.05,alternative="greater")
```
...
h_effect <- seq(.1,.3,.05) #from 0.1 to 0.3 by 0.05
n_effect <- length(h_effect) #5
# power values
p <- seq(.1,.9,.1)
np <- length(p) #9
# create an empty array 9 x 5
samplesize <- array(numeric(n_effect*np), dim=c(n_effect,np))
for (i in 1:np){
  for (j in 1:n_effect){
    result <- pwr.2p.test(n = NULL, h_effect = h_effect[j],
    sig.level = .05, power = p[i],
    alternative = "two.sided")
    samsize[j,i] <- ceiling(result$n_effect)
  }
}
```
# set up graph
...
abline(h=0, v=seq(xrange[1],xrange[2],.02), lty=2,
   col="grey89")
title("Sample Size Estimation for Proportions\n
  Sig=0.05 (Two-tailed)")
...
### PRISMA
The PRISMA statement is useful for understanding the search strategy and the papers removed and retained in the metaanalysis. An example of generating the statement is provided below in R.
```{r prisma}
...
```
### Metaregression
```{r nice-fig3, fig.cap='Metaregression', out.width='80%', fig.asp=.75, fig.align='center'}
...
```{r nice-fig4, fig.cap='Bayesian Forest plot of sensitivity and specificity', out.width='80%', fig.asp=.75, fig.align='center'}
#sensitivity
